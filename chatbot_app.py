...
@app.route("/")
def index():
    # --- CHANGE #1: Capture the return_url from the query string ---
    return_url_from_qualtrics = request.args.get('return_url', '') # Default to empty string if not provided
    
    try:
        if not os.path.exists(os.path.join(app.template_folder, "GPT_fake.html")):
            return "Error: GPT_fake.html not found"
        
        # --- CHANGE #2: Pass the captured URL to the HTML template ---
        response = make_response(render_template("GPT_fake.html", return_url=return_url_from_qualtrics))
...
# This /chat route is UNCHANGED from your original code.
...
def chat():
    ...
    if end_session:
        ...
        # This redirect_url is now a placeholder and will be overridden by the dynamic URL in the JavaScript.
        # But we keep the structure the same as your original code.
        redirect_url = f"https://your.qualtrics.com/jfe/form/SV_XXXXXX?PROLIFIC_PID={prolific_pid}"
        return jsonify({
